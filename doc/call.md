...
```
高地址  USERSTACK = 0x00800000  ─── 栈顶（页上界，未映射之上）
        ┌─────────────────────────────┐
...
再下一字  →  AT_NULL
```
程序支持的参数数量并没有一个固定的“个数”限制，而是受到参数与环境变量总大小的限制。
POSIX 标准规定，限制并不是参数的数量，而是传递给 exec 函数的参数和环境数据的总大小。这个限制由系统的 ARG_MAX 常量控制。
- 在大多数现代 Linux 系统中，ARG_MAX 的默认值通常为 2097152 字节（2MB）。
- 通过在终端执行 getconf ARG_MAX 命令来查看当前系统的具体限制值。

为什么没有固定的参数个数？
- 因为每个参数在内存中存储时，除了字符串本身的长度，还需要额外的开销（如内存对齐、字符串结尾的 \0 终止符等）。
- 如果传入的参数都是极短的字符（如 1, 2, 3），2MB 的空间大约可以容纳2万到3万个参数。
- 如果传入的参数是极长的字符串，能传递的参数个数就会大幅减少。